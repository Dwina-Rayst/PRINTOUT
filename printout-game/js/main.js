...
import { PrintoutUser, offlineSaveBestFloor } from "./auth.js";
...
const flashlight = new THREE.SpotLight(0xffe8c0, 3.2, 22, Math.PI / 6, 0.5, 1.6);
...
function wallMat(seed){
  const c = new THREE.Color().setHSL(0.06 + (seed % 5) * 0.005, 0.15, 0.06 + (seed % 3) * 0.01);
  return new THREE.MeshStandardMaterial({ color: c, roughness: 0.95, metalness: 0.05 });
}

function buildRoom(floorNum){
  const floorMat = new THREE.MeshStandardMaterial({ color: 0x0c0b0a, roughness: 1 });
...
function checkDeath(){
  if (player.hp <= 0) {
    ...
    if (PrintoutUser.offline) offlineSaveBestFloor(PrintoutUser.id, state.floor);
    return true;
  }
  return false;
}
